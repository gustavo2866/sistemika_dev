...
type FormCompletarDialogProps = {
  ...
  selectedEvento: CRMEvento | null;
  onSuccess?: (evento: CRMEvento) => void;
  onError?: (error: unknown) => void;
};

const getContactNameSafe = (item: CRMEvento | null) => {
  if (!item) return "Sin contacto";
  return (
    item.contacto?.nombre_completo ??
    item.contacto?.nombre ??
    (item.contacto_id ? `Contacto #${item.contacto_id}` : "Sin contacto")
  );
};
...
export const FormCompletarDialog = ({
  ...
  const oportunidadId = selectedEvento?.oportunidad_id;
  const { data: oportunidad } = useGetOne(
    "crm/oportunidades",
    { id: oportunidadId ?? 0 },
    { enabled: open && Boolean(oportunidadId) }
  );
  const oportunidadLabel =
    oportunidad?.descripcion_estado ??
    selectedEvento?.oportunidad?.descripcion_estado ??
    "";

  useEffect(() => {
    if (!open) return;
    setResultado("");
    const estadoActual = selectedEvento?.oportunidad?.estado ?? oportunidad?.estado ?? "";
    setNuevoEstadoOportunidad(estadoActual || "");
    setMotivoPerdidaId("");
    setMotivoPerdidaError("");
  }, [open, selectedEvento, oportunidad?.estado]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const estadoActual = selectedEvento.oportunidad?.estado ?? "";
        ...
        <DialogHeader>
          <DialogTitle>
            {selectedEvento?.tipo_evento
              ? `Completar ${getTipoLabel(selectedEvento.tipo_evento)}`
              : "Completar evento"}
          </DialogTitle>
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] text-slate-600">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2">
              ...
              <span>
                Oportunidad:{" "}
                {oportunidadId
                  ? `#${String(oportunidadId).padStart(6, "0")} ${oportunidadLabel}`.trim()
                  : "Sin oportunidad"}
              </span>
            </div>
            <div className="mt-1">
              Estado actual oportunidad:{" "}
              {oportunidad?.estado ?? selectedEvento.oportunidad?.estado ?? "Sin estado"}
            </div>
          </div>
        ) : null}
