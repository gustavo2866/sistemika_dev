...
export const PoInvoiceAgendaBulkAutorizarButton = ({
  disabled,
}: {
  disabled?: boolean;
}) => {
  ...
  const handleOpen = useCallback(async () => {
    ...
    const statusAgendada = await resolveStatusFinByOrden(dataProvider, 2);
    if (!statusAgendada?.id) {
      notify("No se encontró el estado financiero Agendada", { type: "warning" });
      return;
    }
    ...
    const notAllowed = (dataSource ?? []).filter((item) => {
      ...
      if (typeof orden === "number") return Number(orden) !== 2;
      return statusId !== statusAgendada.id;
    });
    if (notAllowed.length > 0) {
      notify("Solo se pueden autorizar facturas en estado Agendada", {
        type: "warning",
      });
      return;
    }
  ...
  const handleConfirm = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [statusAgendada, statusAutorizada] = await Promise.all([
        resolveStatusFinByOrden(dataProvider, 2),
        resolveStatusFinByOrden(dataProvider, 3),
      ]);

      if (!statusAgendada?.id || !statusAutorizada?.id) {
      ...
      for (const item of dataSource ?? []) {
        ...
        await dataProvider.update(resource, {
          id: item.id,
          data: { invoice_status_fin_id: statusAutorizada.id },
          previousData: item,
        });
      }

      notify(`Se autorizaron ${selectedCount} facturas`, { type: "info" });
      refresh();
      onUnselectItems();
      setOpen(false);
    } catch (error) {
      console.error(error);
      notify("No se pudieron autorizar las facturas seleccionadas", {
        type: "warning",
      });
    } finally {
      setLoading(false);
      ...
      <Confirm
        ...
        content={
          <div className="space-y-2 text-sm text-muted-foreground">
            <p>Seguro que deseas autorizar las facturas seleccionadas?</p>
