"use client";

import { BarChart3, ChartNoAxesCombined, RefreshCcw } from "lucide-react";
import { useLocation, useNavigate } from "react-router-dom";
import { Button } from "@/components/ui/button";
import type { PoDashboardDetalleItem } from "./model";
import { usePoDashboard } from "./use-po-dashboard";
import { saveDashboardReturnMarker } from "./return-state";
import {
  DashboardHeader,
  DashboardKpiRow,
  DashboardMainPanel,
  DashboardPrimaryFilters,
  useDashboardHeader,
  useDashboardKpiRow,
  useDashboardMainPanel,
  useDashboardPrimaryFilters,
} from "./components";

const SectionShell = ({
  children,
  className = "",
}: {
  children: React.ReactNode;
  className?: string;
}) => (
  <div
    className={`w-full rounded-xl border border-border/70 bg-gradient-to-br from-slate-50 via-white to-slate-100/70 shadow-sm ${className}`}
  >
    {children}
  </div>
);

export default function PoDashboardList() {
  const navigate = useNavigate();
  const location = useLocation();
  const returnTo = `${location.pathname}${location.search}`;

  const handleMobileBack = () => {
    if (typeof window !== "undefined" && window.history.length > 1) {
      navigate(-1);
      return;
    }
    navigate("/po-orders");
  };

  const {
    periodType,
    filters,
    dashboardData,
    dashboardLoading,
    previousPeriodData,
    detailKpi,
    detailData,
    detailLoading,
    selectedAlertKey,
    showAdditionalFilters,
    showKpis,
    periodTrendData,
    solicitanteOptions,
    proveedorOptions,
    tipoSolicitudOptions,
    departamentoOptions,
    tipoCompraOptions,
    alertItems,
    selectorData,
    hasMoreDetail,
    applyRange,
    handleFilterChange,
    setDetailPage,
    setShowAdditionalFilters,
    setShowKpis,
    resetFilters,
    refreshDashboard,
    selectAlert,
    selectDetailKpi,
  } = usePoDashboard();

  const saveDashboardContext = (extra?: {
    orderId?: string | number | null;
    refreshAll?: boolean;
    deleted?: boolean;
  }) => {
    saveDashboardReturnMarker(returnTo, {
      savedAt: Date.now(),
      orderId: extra?.orderId,
      refreshAll: extra?.refreshAll ?? false,
      deleted: extra?.deleted,
      filters,
      periodType,
      detailKpi,
      selectedAlertKey,
      showAdditionalFilters,
    });
  };

  const headerModel = useDashboardHeader({
    dashboardLoading,
    onMobileBack: handleMobileBack,
  });

  const primaryFiltersModel = useDashboardPrimaryFilters({
    periodType,
    filters,
    showAdditionalFilters,
    solicitanteOptions,
    proveedorOptions,
    tipoSolicitudOptions,
    departamentoOptions,
    tipoCompraOptions,
    onApplyRange: applyRange,
    onSolicitanteChange: (value) => handleFilterChange("solicitanteId", value),
    onProveedorChange: (value) => handleFilterChange("proveedorId", value),
    onTipoSolicitudChange: (value) => handleFilterChange("tipoSolicitudId", value),
    onDepartamentoChange: (value) => handleFilterChange("departamentoId", value),
    onTipoCompraChange: (value) => handleFilterChange("tipoCompra", value),
    onToggleAdditionalFilters: () =>
      setShowAdditionalFilters((current) => !current),
    onResetFilters: resetFilters,
  });

  const kpiRowModel = useDashboardKpiRow({
    dashboardData,
    previousPeriodData,
    periodTrendData,
  });

  const handleOpenOrder = (item: PoDashboardDetalleItem) => {
    if (!item.order?.id) return;
    saveDashboardContext({ orderId: item.order.id });
    navigate(`/po-orders/${item.order.id}`, {
      state: { returnTo },
    });
  };

  const createOrderPath = `/po-orders/create?returnTo=${encodeURIComponent(returnTo)}`;

  const mainPanelModel = useDashboardMainPanel({
    selectorData,
    detailKpi,
    selectedAlertKey,
    detailData,
    detailLoading,
    hasMoreDetail,
    onLoadMore: () => setDetailPage((current) => current + 1),
    alertItems,
    onSelectAlert: selectAlert,
    onSelectStatusCard: selectDetailKpi,
    onOpenOrder: handleOpenOrder,
    onNavigate: (path) => {
      const isManagedDestination =
        path === createOrderPath ||
        path.startsWith("/po-orders") ||
        path.startsWith("/po-orders-approval") ||
        path.startsWith("/po-invoices");
      if (isManagedDestination) {
        saveDashboardContext({ refreshAll: path === createOrderPath });
      }
      navigate(path);
    },
    createOrderPath,
  });

  const kpiToggleLabel = showKpis ? "Ocultar KPIs" : "Mostrar KPIs";

  return (
    <div className="mx-auto flex h-full w-full max-w-5xl min-h-0 flex-1 flex-col gap-3 overflow-y-auto overscroll-y-contain px-0 pt-2 pb-3 xl:max-w-6xl 2xl:max-w-7xl sm:px-2 sm:pt-4 sm:pb-4">
      <section className="w-full">
        <SectionShell className="pt-2 pb-1 sm:pt-3 sm:pb-2">
          <div className="relative space-y-0.5 px-0 pt-1 pb-0 sm:space-y-1.5 sm:px-5 sm:pt-2 sm:pb-1">
            <DashboardHeader {...headerModel} />
            <DashboardPrimaryFilters {...primaryFiltersModel} />
          </div>
        </SectionShell>
      </section>

      <section className="w-full">
        <SectionShell className="px-3 py-2 sm:px-5 sm:py-3">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground sm:text-[11px]">
              <ChartNoAxesCombined className="h-3.5 w-3.5 text-primary" />
              <span>KPIs</span>
            </div>
            <div className="flex items-center gap-1">
              <Button
                type="button"
                variant="ghost"
                size="sm"
                className="h-8 gap-1 px-2 text-[10px] text-muted-foreground sm:text-[11px]"
                onClick={() => setShowKpis((current) => !current)}
                aria-label={kpiToggleLabel}
                title={kpiToggleLabel}
              >
                {showKpis ? (
                  <ChartNoAxesCombined className="h-3.5 w-3.5" />
                ) : (
                  <BarChart3 className="h-3.5 w-3.5" />
                )}
                <span>{showKpis ? "Ocultar" : "Mostrar"}</span>
              </Button>
              <Button
                type="button"
                variant="outline"
                size="sm"
                className="h-7 w-7 shrink-0 px-0 text-muted-foreground sm:h-7 sm:w-7"
                onClick={refreshDashboard}
                title="Actualizar"
                aria-label="Actualizar"
              >
                <RefreshCcw className="h-3 w-3" />
              </Button>
            </div>
          </div>
          {showKpis ? (
            <div className="mt-3">
              <DashboardKpiRow {...kpiRowModel} />
            </div>
          ) : null}
        </SectionShell>
      </section>

      <section className="flex w-full flex-col">
        <SectionShell className="px-0 py-0 sm:px-4 sm:py-4">
          <div className="flex flex-col px-0 py-0">
            <DashboardMainPanel {...mainPanelModel} />
          </div>
        </SectionShell>
      </section>
    </div>
  );
}
