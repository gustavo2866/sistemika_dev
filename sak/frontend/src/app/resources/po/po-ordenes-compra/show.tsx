/**
 * Vista de solo lectura para Ordenes de Compra.
 *
 * Layout identico a po-solicitudes, con cambio de nombre en cabecera.
 */
...
import type { PoOrdenCompra, PoOrdenCompraDetalle } from "./model";
...
const PoOrdenCompraDetalleTable = ({
  detalles,
}: {
            ...
            {detalles.map((detalle) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="w-[70px] px-1.5 py-1 text-right text-[8px] sm:w-[80px] sm:px-2.5 sm:py-2 sm:text-[9px]">
                    {CURRENCY_FORMATTER.format(Number(detalle.total_linea ?? 0))}
                  </td>
                </tr>
              );
            })}
...
const PoOrdenCompraShowContent = () => {
  ...
  const tipoCompraLabel =
    ...
    record.tipo_compra || "-";
  const numero = Number.isFinite(record.id)
    ...
    <div className="space-y-4">
      ...
      <Card className="p-3 sm:p-4">
        {record.detalles?.length ? (
          <PoOrdenCompraDetalleTable detalles={record.detalles} />
        ) : (
...
const PoOrdenCompraShowTitle = () => {
  const record = useRecordContext<PoOrdenCompra>();
  if (!record) return "Ordenes de Compra";
...
export const PoOrdenCompraShow = () => (
  <Show className="w-full max-w-3xl" title={<PoOrdenCompraShowTitle />}>
    <div className="print-root w-full max-w-3xl">
      <PoOrdenCompraShowContent />
    </div>
  </Show>
);
