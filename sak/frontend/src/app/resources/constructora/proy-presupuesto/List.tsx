...
export const ProyPresupuestoList = ({
  ...
  const returnTo = isEmbedded ? `${location.pathname}${location.search}` : undefined;
...
export const ProyPresupuestoListBody = ({
  ...
  const rowClick = returnTo
    ? (id: string | number, resource: string) =>
        `/${resource}/${id}?returnTo=${encodeURIComponent(returnTo)}`
    : "edit";
  ...
  return (
    <ResponsiveDataTable
      ...
      mobileConfig={{
        primaryField: "fecha",
        secondaryFields: ["proyecto_id", "importe", "materiales", "herramientas", "horas"],
      }}
    ...
    >
      ...
      <ListColumn source="herramientas" label="Herram." className="w-[64px] xl:w-[70px] text-right">
        <PresupuestoMillionsField source="herramientas" />
      </ListColumn>
