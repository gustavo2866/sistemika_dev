...
import {
  CONSTRUCTORA_SETUP_GROUPS,
  CONSTRUCTORA_SETUP_ITEMS,
  getConstructoraSetupItem,
} from "./constructoraSetupRegistry";

const setupBasePath = "/constructora/setup";
...
const getSetupGroupKeyByItem = (itemKey?: string | null) =>
  CONSTRUCTORA_SETUP_GROUPS.find((group) =>
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" className="w-64">
        {CONSTRUCTORA_SETUP_GROUPS.map((group, groupIndex) => (
          <div key={group.key}>
...
const SetupDesktopGroupMenu = ({
  ...
  <div className="hidden md:flex md:flex-wrap md:items-center md:gap-1 rounded-xl border border-border/70 bg-muted/20 p-1.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.5)]">
    {CONSTRUCTORA_SETUP_GROUPS.map((group) => {
            ...
            {group.items.map((item) => {
              ...
              return (
                <DropdownMenuItem
                  ...
                  asChild
                >
                  <Link to={getSetupItemPath(item)}>{item.label}</Link>
                </DropdownMenuItem>
              );
            })}
          </DropdownMenuContent>
...
const ConstructoraSetupContent = ({
  item,
  view,
  recordId,
}: {
...
export const ConstructoraSetupPage = () => {
  const location = useLocation();
  const { selectedKey, currentView, recordId } = getSetupRouteState(location.pathname);
  const selectedItem = getConstructoraSetupItem(selectedKey);
  const defaultItem = CONSTRUCTORA_SETUP_ITEMS[0] ?? null;
  ...
  return (
    <div className="max-w-5xl px-2 py-3 sm:p-6">
      <SetupLayout
        header={
          <SetupContentHeader
            eyebrowLabel="Constructora / Setup"
            title={false}
            ...
            actions={
              <div className="space-y-2">
                <div className="md:hidden">
                  <SetupOptionsMenu currentKey={selectedItem.key} />
                </div>
                <SetupDesktopGroupMenu
...
export default ConstructoraSetupPage;
