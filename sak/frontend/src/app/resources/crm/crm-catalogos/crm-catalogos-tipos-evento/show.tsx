...
const EstadoTipoEventoBadge = ({
  ...
  <Badge
    variant="secondary"
    className={getTipoEventoBadgeClass(activo)}
  >
    {getTipoEventoEstadoLabel(activo)}
  </Badge>
);
...
const CRMTipoEventoShowTitle = () => {
  const record = useRecordContext<CRMTipoEvento>();
  if (!record) return "Tipo de evento CRM";

  return (
    <div className="flex flex-wrap items-center gap-2">
      <span>Tipo de evento CRM</span>
      <Badge variant="outline" className="text-[11px]">
        #{String(record.id ?? "").padStart(6, "0")}
      </Badge>
      <EstadoTipoEventoBadge activo={record.activo} />
    </div>
  );
};

// Renderiza el contenido de solo lectura del registro seleccionado.
const CRMTipoEventoShowContent = () => {
  const record = useRecordContext<CRMTipoEvento>();
...
export const CRMTipoEventoShow = () => (
  <Show
    className="w-full max-w-2xl"
    title={<CRMTipoEventoShowTitle />}
    actions={<FormOrderEditButton />}
  >
    <div className="w-full max-w-2xl">
      <CRMTipoEventoShowContent />
    </div>
  </Show>
);
