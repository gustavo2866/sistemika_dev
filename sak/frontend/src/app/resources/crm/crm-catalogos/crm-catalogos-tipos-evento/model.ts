...
export type CRMTipoEventoRecord = CRMTipoEvento & {
  id: number | string;
};
...
export const crmTipoEventoSchema = z.object({
...
export type CRMTipoEventoFormValues = z.infer<typeof crmTipoEventoSchema>;

// Centraliza los valores iniciales del formulario del recurso.
export const CRM_TIPO_EVENTO_DEFAULTS: CRMTipoEventoFormValues = {
  codigo: "",
  nombre: "",
  descripcion: "",
  activo: true,
...
// Devuelve la etiqueta visible del estado de un tipo de evento.
export const getTipoEventoEstadoLabel = (activo?: boolean | null) =>
  activo === false ? "Inactivo" : "Activo";

// Devuelve las clases visuales del badge segun el estado actual.
export const getTipoEventoBadgeClass = (activo?: boolean | null) =>
  activo === false
    ? "border border-slate-300 bg-slate-100 text-slate-700"
    : "border border-emerald-200 bg-emerald-100 text-emerald-700";

// Normaliza el payload antes de enviarlo al backend.
export const normalizeTipoEventoPayload = (
  data: Partial<CRMTipoEventoFormValues>,
): CRMTipoEventoFormValues => ({
