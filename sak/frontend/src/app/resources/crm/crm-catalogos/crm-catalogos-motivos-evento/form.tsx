...
import {
  CRM_MOTIVO_EVENTO_DEFAULTS,
  VALIDATION_RULES,
  crmMotivoEventoSchema,
  type CRMMotivoEventoFormValues,
} from "./model";

//#region Componentes auxiliares del formulario

// Renderiza los campos principales editables del motivo de evento.
const CamposPrincipalesMotivoEvento = () => (
...
const AyudaMotivoEvento = () => (
  <div className="rounded-md border border-muted/60 bg-muted/30 p-3 text-xs text-muted-foreground">
    ...
    para registrar eventos dentro del flujo CRM.
  </div>
);
...
export const CRMMotivoEventoForm = () => (
  <SimpleForm<CRMMotivoEventoFormValues>
    className="w-full max-w-2xl"
    // ra-core tipa resolver como Resolver<FieldValues>, igual que otros resources.
    resolver={zodResolver(crmMotivoEventoSchema) as any}
    toolbar={<FormOrderToolbar saveProps={{ variant: "secondary" }} />}
    defaultValues={CRM_MOTIVO_EVENTO_DEFAULTS}
  >
    <SectionBaseTemplate
      title="Datos del motivo de evento"
      main={<CamposPrincipalesMotivoEvento />}
      optional={<AyudaMotivoEvento />}
      defaultOpen
    />
  </SimpleForm>
);
