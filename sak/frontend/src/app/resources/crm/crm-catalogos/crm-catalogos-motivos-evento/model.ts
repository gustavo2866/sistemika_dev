...
export type CRMMotivoEventoRecord = CRMMotivoEvento & {
  id: number | string;
};
...
export const crmMotivoEventoSchema = z.object({
...
export type CRMMotivoEventoFormValues = z.infer<typeof crmMotivoEventoSchema>;

// Centraliza los valores iniciales del formulario del recurso.
export const CRM_MOTIVO_EVENTO_DEFAULTS: CRMMotivoEventoFormValues = {
  codigo: "",
  nombre: "",
  descripcion: "",
  activo: true,
...
// Devuelve la etiqueta visible del estado de un motivo de evento.
export const getMotivoEventoEstadoLabel = (activo?: boolean | null) =>
  activo === false ? "Inactivo" : "Activo";

// Devuelve las clases visuales del badge segun el estado actual.
export const getMotivoEventoBadgeClass = (activo?: boolean | null) =>
  activo === false
    ? "border border-slate-300 bg-slate-100 text-slate-700"
    : "border border-emerald-200 bg-emerald-100 text-emerald-700";

// Normaliza el payload antes de enviarlo al backend.
export const normalizeMotivoEventoPayload = (
  data: Partial<CRMMotivoEventoFormValues>,
): CRMMotivoEventoFormValues => ({
