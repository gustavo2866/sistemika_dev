...
export type CRMMotivoPerdidaRecord = CRMMotivoPerdida & {
  id: number | string;
};
...
export const crmMotivoPerdidaSchema = z.object({
...
export type CRMMotivoPerdidaFormValues = z.infer<typeof crmMotivoPerdidaSchema>;

// Centraliza los valores iniciales del formulario del recurso.
export const CRM_MOTIVO_PERDIDA_DEFAULTS: CRMMotivoPerdidaFormValues = {
  codigo: "",
  nombre: "",
  descripcion: "",
  activo: true,
...
// Devuelve la etiqueta visible del estado de un motivo de perdida.
export const getMotivoPerdidaEstadoLabel = (activo?: boolean | null) =>
  activo === false ? "Inactivo" : "Activo";

// Devuelve las clases visuales del badge segun el estado actual.
export const getMotivoPerdidaBadgeClass = (activo?: boolean | null) =>
  activo === false
    ? "border border-slate-300 bg-slate-100 text-slate-700"
    : "border border-emerald-200 bg-emerald-100 text-emerald-700";

// Normaliza el payload antes de enviarlo al backend.
export const normalizeMotivoPerdidaPayload = (
  data: Partial<CRMMotivoPerdidaFormValues>,
): CRMMotivoPerdidaFormValues => ({
