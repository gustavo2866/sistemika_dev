...
export type CRMCondicionPagoRecord = CRMCondicionPago & {
  id: number | string;
};
...
export const crmCondicionPagoSchema = z.object({
...
export type CRMCondicionPagoFormValues = z.infer<typeof crmCondicionPagoSchema>;

// Centraliza los valores iniciales del formulario del recurso.
export const CRM_CONDICION_PAGO_DEFAULTS: CRMCondicionPagoFormValues = {
  codigo: "",
  nombre: "",
  descripcion: "",
  activo: true,
...
// Devuelve la etiqueta visible del estado de una condicion de pago.
export const getCondicionPagoEstadoLabel = (activo?: boolean | null) =>
  activo === false ? "Inactiva" : "Activa";

// Devuelve las clases visuales del badge segun el estado actual.
export const getCondicionPagoBadgeClass = (activo?: boolean | null) =>
  activo === false
    ? "border border-slate-300 bg-slate-100 text-slate-700"
    : "border border-emerald-200 bg-emerald-100 text-emerald-700";

// Normaliza el payload antes de enviarlo al backend.
export const normalizeCondicionPagoPayload = (
  data: Partial<CRMCondicionPagoFormValues>,
): CRMCondicionPagoFormValues => ({
