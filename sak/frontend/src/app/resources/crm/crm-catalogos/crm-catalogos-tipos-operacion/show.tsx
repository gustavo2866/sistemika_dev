...
const EstadoTipoOperacionBadge = ({
  activo,
}: {
  activo?: boolean | null;
}) => (
  <Badge
    variant="secondary"
    className={getTipoOperacionBadgeClass(activo)}
  >
    {getTipoOperacionEstadoLabel(activo)}
  </Badge>
);
...
const CRMTipoOperacionShowTitle = () => {
  const record = useRecordContext<CRMTipoOperacion>();
  if (!record) return "Tipo de operacion CRM";

  return (
    <div className="flex flex-wrap items-center gap-2">
      <span>Tipo de operacion CRM</span>
      <Badge variant="outline" className="text-[11px]">
        #{String(record.id ?? "").padStart(6, "0")}
      </Badge>
      <EstadoTipoOperacionBadge activo={record.activo} />
    </div>
  );
};

// Renderiza el contenido de solo lectura del registro seleccionado.
const CRMTipoOperacionShowContent = () => {
  const record = useRecordContext<CRMTipoOperacion>();
...
export const CRMTipoOperacionShow = () => (
  <Show
    className="w-full max-w-2xl"
    title={<CRMTipoOperacionShowTitle />}
    actions={<FormOrderEditButton />}
  >
    <div className="w-full max-w-2xl">
      <CRMTipoOperacionShowContent />
    </div>
  </Show>
);
