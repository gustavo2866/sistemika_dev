...
export type CRMOrigenLeadRecord = CRMOrigenLead & {
  id: number | string;
};
...
export const crmOrigenLeadSchema = z.object({
...
export type CRMOrigenLeadFormValues = z.infer<typeof crmOrigenLeadSchema>;

// Centraliza los valores iniciales del formulario del recurso.
export const CRM_ORIGEN_LEAD_DEFAULTS: CRMOrigenLeadFormValues = {
  codigo: "",
  nombre: "",
  descripcion: "",
  activo: true,
...
// Devuelve la etiqueta visible del estado de un origen de lead.
export const getOrigenLeadEstadoLabel = (activo?: boolean | null) =>
  activo === false ? "Inactivo" : "Activo";

// Devuelve las clases visuales del badge segun el estado actual.
export const getOrigenLeadBadgeClass = (activo?: boolean | null) =>
  activo === false
    ? "border border-slate-300 bg-slate-100 text-slate-700"
    : "border border-emerald-200 bg-emerald-100 text-emerald-700";

// Normaliza el payload antes de enviarlo al backend.
export const normalizeOrigenLeadPayload = (
  data: Partial<CRMOrigenLeadFormValues>,
): CRMOrigenLeadFormValues => ({
