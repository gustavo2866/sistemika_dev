...
type InmobiliariaSetupMenuItem = {
  key: string;
  label: string;
};

type InmobiliariaSetupGroup = {
  key: string;
  label: string;
  items: InmobiliariaSetupMenuItem[];
};

const setupBasePath = "/inmobiliaria-setup";

const INMOBILIARIA_SETUP_GROUPS: InmobiliariaSetupGroup[] = [
  {
    key: "propiedades",
    label: "Propiedades",
    items: [
      { key: "tipos-propiedad", label: "Tipos" },
      { key: "propiedades-status", label: "Estados" },
    ],
  },
  {
    key: "contratos",
...
const getSetupGroupKeyByItem = (itemKey?: string | null) =>
  INMOBILIARIA_SETUP_GROUPS.find((group) =>
        ...
        {groups.map((group, groupIndex) => (
          <div key={group.key}>
            ...
            {group.items.map((groupItem) => {
              const item = getInmobiliariaSetupItem(groupItem.key);
              if (!item) return null;
...
const InmobiliariaSetupContent = ({
  item,
  view,
  recordId,
}: {
...
export const InmobiliariaSetupPage = () => {
  ...
  const defaultItem = INMOBILIARIA_SETUP_ITEMS[0] ?? null;
  ...
  return (
    <div className="max-w-5xl px-2 py-3 sm:p-6">
      <SetupLayout
        header={
          <SetupContentHeader
            eyebrowLabel="Inmobiliaria / Setup"
            title={false}
            ...
            actions={
              <div className="space-y-2">
                <div className="md:hidden">
                  <SetupOptionsMenu
                    groups={INMOBILIARIA_SETUP_GROUPS}
