...
export const VacanciaTimeline = ({
  vacancia,
  title = "Ultimo ciclo de vacancia",
}: VacanciaTimelineProps) => {
  if (!vacancia) {
    return (
      <Card>
        <CardHeader>
        ...
        <CardContent className="text-sm text-muted-foreground">
          La propiedad aun no registra vacancias.
        </CardContent>
      </Card>
    );
  }
            ...
            return (
              <li key={step.key} className="flex gap-4">
                <div className="flex flex-col items-center">
                  ...
                  />
                  {step.key !==
                    VACANCIA_STATE_STEPS[VACANCIA_STATE_STEPS.length - 1].key && (
                ...
                <div className="flex-1 space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {formatDate(vacancia[dateKey] as string | undefined)}
                  </p>
                  {vacancia[commentKey] ? (
                    <p className="rounded bg-muted px-3 py-2 text-sm">
                      {vacancia[commentKey] as string}
                    </p>
                  ) : (
