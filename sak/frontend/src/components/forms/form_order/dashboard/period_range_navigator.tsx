...
export const PeriodRangeNavigator = ({
  ...
  const rangeLabel = useMemo(() => {
    return `${new Date(alignedValue.startDate).toLocaleDateString(
      "es-AR",
    )} - ${new Date(alignedValue.endDate).toLocaleDateString("es-AR")}`;
  }, [alignedValue]);
  ...
  return (
    <div className={cn("flex flex-col gap-1", className)}>
      ...
      <div
        className={cn(
          "flex w-full flex-wrap items-center gap-1 rounded-lg border border-border/70 bg-card px-3 py-2 text-sm shadow-sm sm:w-fit",
        )}
      >
        <Button
          type="button"
        ...
        <Select
          value={periodType}
          onValueChange={(value) => handleSelectChange(value as PeriodType)}
        >
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button
              variant="outline"
              className="h-8 w-full justify-between sm:min-w-[180px] sm:w-auto"
            >
              {rangeLabel}
            </Button>
          </PopoverTrigger>
