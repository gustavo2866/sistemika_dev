...
ESTADOS_TERMINALES = {"vencido", "rescindido", "renovado"}
...
def activar_contrato(id: int, session: Session = Depends(get_session)):
    ...
    # Validar que no haya otro vigente para la misma propiedad
...
# --- Renovar: vigente|vencido → renovado + nuevo borrador ---
...
def renovar_contrato(id: int, session: Session = Depends(get_session)):
    ...
    if contrato.estado not in ("vigente", "vencido"):
        raise HTTPException(
            status_code=400,
            detail=f"Solo se puede renovar un contrato 'vigente' o 'vencido'. Estado actual: '{contrato.estado}'",
        )
    ...
    session.add(nuevo)
    session.flush()  # obtener nuevo.id antes del commit

    # Marcar el contrato original como renovado
    contrato.estado = "renovado"
    contrato.contrato_renovado_id = nuevo.id
    contrato.updated_at = datetime.now(UTC)
    session.add(contrato)
